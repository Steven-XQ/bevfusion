    ...
    def forward(self, x):
        ...
        for i, expert in enumerate(self.experts):
            ...
            expert_outputs.append(
                [expert_output_1 * routing_probs[:, i].unsqueeze(1).unsqueeze(2).unsqueeze(3),
                 expert_output_2 * routing_probs[:, i].unsqueeze(1).unsqueeze(2).unsqueeze(3),
                 expert_output_3 * routing_probs[:, i].unsqueeze(1).unsqueeze(2).unsqueeze(3)]
            )
        
        # 专家的结果相加
        final_output_1 = sum([output[0] for output in expert_outputs])
        final_output_2 = sum([output[1] for output in expert_outputs])
        final_output_3 = sum([output[2] for output in expert_outputs])
...
class MoENetwork(BaseModule):
    def __init__(self, num_experts=2, in_channels=3, experts_cfg=None, router=None, init_cfg=None):
